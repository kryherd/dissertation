...
ashby1 <- read.csv("Ashby_concat_fall2018.csv")
ashby2 <- read.csv("Ashby_concat_03-08-19_1626.csv")
colnames(ashby1)[3] <- "Run"
colnames(ashby1)[6] <- "key_pressed"
ashby <- rbind(ashby1,ashby2)
sloutsky <- read.csv("Sloutsky_concat_03-08-19_1638.csv")
taxthem <- read.csv("taxthem_concat_03-08-19_1637.csv")
tol <- read.csv("tol_concat_03-08-19_1639.csv")
switcher <- read.csv("switcher_concat_03-08-19_1639.csv")
flanker <- read.csv("flanker_concat_03-08-19_1639.csv")
ravens <- read.csv("ravens_3-8-19.csv")
...
id <- full_join(nd_celf, ravens, by = c("subnum" = "Subject")) %>%
...
anova(m1,m3)
anova(m3)
m4 <- lmer(ashby.t ~ ravens_tf + System * (flanker_eff_cs + switch_eff_tf + tol_acc_tf + nd_tf) + (1|Subject), data = ashby_dat)
anova(m3,m4)
anova(m4)
...
anova(m1,m3)
anova(m3)
m4 <- lmer(sloutsky.t ~ ravens_tf + System * (flanker_eff_cs + switch_eff_tf + tol_acc_tf + nd_tf) + (1|Subject), data = sloutsky_dat)
anova(m3,m4)
anova(m4)
...
anova(m1,m3)
anova(m3)
m4 <- lmer(tt.t ~ ravens_tf + System * (flanker_eff_cs + switch_eff_tf + tol_acc_tf + nd_tf) + (1|Subject), data = tt_dat)
anova(m3,m4)
anova(m4)
...
ht_mod <- lm(sloutsky.t ~ ravens_tf + flanker_eff_cs + switch_eff_tf + tol_acc_tf, data = sloutsky_dat, subset = ht)
summary(ht_mod)

as_mod <- lm(sloutsky.t ~ ravens_tf + flanker_eff_cs + switch_eff_tf + tol_acc_tf, data = sloutsky_dat, subset = as)
summary(as_mod)
...
#anova(m1,m2)
#anova(m2)
m3 <- lmer(tt.t ~ System + flanker_eff_cs + switch_eff_tf + tol_acc_tf + nd_tf  + (1|Subject), data = tt_dat)
anova(m1,m3)
anova(m3)
m4 <- lmer(tt.t ~ ravens_tf + System * (flanker_eff_cs + switch_eff_tf + tol_acc_tf + nd_tf) + (1|Subject), data = tt_dat)
anova(m3,m4)
...
ggplot(tt_dat, aes(flanker_eff_cs, tt.t)) + geom_point() + theme_bw() + stat_smooth(se = FALSE, method = "lm") + xlab("Flanker Effect") + ylab("TT Reaction Time") + ggtitle("??") + facet_grid(.~System)
