...
ashby_acc <- acc_wide %>%
  dplyr::select(Subject, System, ashby, ashby.t) %>%
  na.omit 

sloutsky_acc <- acc_wide %>%
  dplyr::select(Subject, System, sloutsky, sloutsky.t) %>%
  na.omit

tt_acc <- acc_wide %>%
  dplyr::select(Subject, System, tt, tt.t) %>%
  na.omit
```
...
for (i in 2:12){
  name <- names(tol_nd)[i]
  plot <- hist(as.data.frame(tol_nd)[,i], main = name)
  plot
  dago <- fBasics::dagoTest(as.data.frame(tol_nd)[,i])
...
vars_tf <- c("switch_eff", "tol_acc", "tol_time", "nd_ss")
pp_md_tf <- preProcess(as.data.frame(tol_nd)[,vars_tf], method = c("center", "scale", "YeoJohnson"), na.remove=T)
tf_data <- predict(pp_md_tf, as.data.frame(tol_nd)[,vars_tf])
names(tf_data) <- c("switch_eff_tf", "tol_acc_tf", "tol_time_tf", "nd_tf")
tol_nd <- cbind(as.data.frame(tol_nd),tf_data)

# scale and center flanker
tol_nd$flanker_eff_cs <- scale(tol_nd$flanker_eff)[,1]
```


```{r cor_plot}
# correlation matrix
cor_plot_dat <- dplyr::select(tol_nd, flanker_eff_cs, switch_eff_tf,tol_acc_tf, tol_time_tf, nd_tf)
...
```{r ashby_acc}
ashby_acc_id <- merge(ashby_acc, tol_nd, by.x = "Subject", by.y = "subnum", all.x = TRUE, all.y = FALSE)

ashby_dat <- ashby_acc_id %>%
  dplyr::select(Subject, System, ashby.t, flanker_eff_cs, switch_eff_tf, tol_acc_tf, tol_time_tf, nd_tf) %>% 
  na.omit %>%
...
anova(m0,m1)
m2 <- lmer(ashby.t ~ System + flanker_eff_cs + switch_eff_tf + tol_time_tf + nd_tf + (1|Subject), data = ashby_dat)
anova(m1,m2)
anova(m2)
...
```{r sloutsky_acc}
sloutsky_acc_id <- merge(sloutsky_acc, tol_nd, by.x = "Subject", by.y = "subnum", all.x = TRUE, all.y = FALSE)

sloutsky_dat <- sloutsky_acc_id %>%
  dplyr::select(Subject, System, sloutsky.t, flanker_eff_cs, switch_eff_tf, tol_acc_tf, tol_time_tf, nd_tf) %>% 
  na.omit %>%
  head(264) # take first 132 with full data
...
anova(m0,m1)
m2 <- lmer(sloutsky.t ~ System + flanker_eff_cs + switch_eff_tf + tol_time_tf + nd_tf + (1|Subject), data = sloutsky_dat)
anova(m1,m2)
anova(m2)
...
```{r tt_acc}
tt_acc_id <- merge(tt_acc, tol_nd, by.x = "Subject", by.y = "subnum", all.x = TRUE, all.y = FALSE)

tt_dat <- tt_acc_id %>%
  dplyr::select(Subject, System, tt.t, flanker_eff_cs, switch_eff_tf, tol_acc_tf, tol_time_tf, nd_tf) %>% 
  na.omit %>%
  head(264) # take first 132 with full data
...
anova(m0,m1)
m2 <- lmer(tt.t ~ System + flanker_eff_cs + switch_eff_tf + tol_time_tf + nd_tf + (1|Subject), data = tt_dat)
anova(m1,m2)
anova(m2)
...
```{r ashby_rt}
ashby_rt_id <- merge(ashby_rt, tol_nd, by.x = "Subject", by.y = "subnum", all.x = TRUE, all.y = FALSE)
...
```{r sloutsky_rt}
sloutsky_rt_id <- merge(sloutsky_rt, tol_nd, by.x = "Subject", by.y = "subnum", all.x = TRUE, all.y = FALSE)
...
```{r tt_rt}
tt_rt_id <- merge(tt_rt, tol_nd, by.x = "Subject", by.y = "subnum", all.x = TRUE, all.y = FALSE)
