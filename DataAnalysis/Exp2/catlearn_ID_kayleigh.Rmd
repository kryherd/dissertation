...
ashby_dat %>%
  ...
  ggplot(., aes(nd_tf, ashby.t)) + geom_point() + theme_bw() + facet_grid(.~System) + stat_smooth(method = "lm", se = FALSE) + ggtitle("Relationships Between Vocabulary and Ashby Accuracy") + xlab("Nelson-Denny Vocabulary (scaled)") + ylab("Ashby Accuracy (scaled)") + theme(plot.title = element_text(hjust = 0.5))
...
tt_dat %>%
  ...
  ggplot(., aes(tol_acc_tf, tt.t_m)) + geom_point() + theme_bw() + stat_smooth(se = FALSE, method = "lm") + xlab("Tower of London Accuracy (scaled)") + ylab("Tax/Them Accuracy (scaled)") + ggtitle("Main Effect of Planning on Tax/Them Accuracy")
...
ashby_dat %>%
  mutate(System = ifelse(System == "associative", "Associative", "Hypothesis-Testing")) %>%
  ggplot(., aes(nd_tf, ashby.t)) + geom_point() + theme_bw() + facet_grid(.~System) + stat_smooth(method = "lm", se = FALSE)  + ggtitle("Relationships Between Vocabulary and Ashby RT") + xlab("Nelson-Denny Vocab (scaled)") + ylab("Ashby Reaction Time (scaled)") + theme(plot.title = element_text(hjust = 0.5))
...
sloutsky_dat %>%
  mutate(System = ifelse(System == "associative", "Associative", "Hypothesis-Testing")) %>%
  ggplot(., aes(tol_acc_tf, sloutsky.t)) + geom_point() + theme_bw() + facet_grid(.~System) + stat_smooth(method = "lm", se = FALSE)  + ggtitle("Relationships Between Planning and Sloutsky RT") + xlab("Tower of London Accuracy (scaled)") + ylab("Sloutsky Reaction Time (scaled)") + theme(plot.title = element_text(hjust = 0.5))
...
tt_dat %>%
  mutate(System = ifelse(System == "associative", "Associative", "Hypothesis-Testing")) %>%
  ggplot(., aes(flanker_eff_cs, tt.t)) + geom_point() + theme_bw() + facet_grid(.~System) + stat_smooth(method = "lm", se = FALSE)  + ggtitle("Relationships Between Inhibition and Tax/Them RT") + xlab("Flanker Effect (scaled)") + ylab("Tax/Them Reaction Time (scaled)") + theme(plot.title = element_text(hjust = 0.5))
