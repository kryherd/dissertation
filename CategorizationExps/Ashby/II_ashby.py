...
info = {'ID Number':''}
dlg = gui.DlgFromDict(info, title = 'Ashby Task Startup - II')
if not dlg.OK:
    core.quit()
...
prefix = 'AshbyTask-II-sub%s' % (info['ID Number'])
...
# save data
np.savetxt("results/" + prefix+"_results.tsv", data, fmt='%s', delimiter='\t', newline='\n', header='', footer='', comments='# ')
